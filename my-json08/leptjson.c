...
#ifndef LEPT_PARSE_ARRAY_INIT_SIZE
#define LEPT_PARSE_ARRAY_INIT_SIZE 256
#endif
...
static int lept_parse_array(lept_context* c, lept_value* v) {
    ...
    int ret;
    EXPECT(c, '[');
    lept_set_array(v, LEPT_PARSE_ARRAY_INIT_SIZE);
...
lept_value* lept_get_object_value(const lept_value* v, size_t index) {
    assert(v != NULL && (index < v->u.o.size && index >= 0) && v->type == LEPT_OBJECT);
    ...
    switch(lhs->type) {
        ...
        case LEPT_OBJECT:
            ...
            for(i = 0; i < lhs->u.o.size; i++) {
                index = lept_find_object_index(&rhs->u.o.m[i].v, lhs->u.o.m[i].k, lhs->u.o.m[i].klen);
    ...
    switch(src->type) {
        ...
        case LEPT_ARRAY:
            ...
            for(i = 0; i < src->u.a.size; i++) {
                lept_copy(&dst->u.a.e[i], &src->u.a.e[i]); /* 不太确定 */
            }
...
lept_value* lept_insert_array_element(lept_value* v, size_t index) {
    assert(v != NULL && v->type == LEPT_ARRAY && index <= v->u.a.size + 1);
    size_t i;
    if(v->u.a.size == v->u.a.capacity)
        lept_reserve_array(v, v->u.a.capacity == 0 ? 1: v->u.a.capacity * 2);
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count) {
    assert(v != NULL && v->type == LEPT_ARRAY && index <= v->u.a.size && count > 0);
    size_t i;
    size_t j = count; 
    for(i = index-1; i < v->u.a.size && j >= 0; i++, j--) {
        lept_free(&v->u.a.e[i]);
    }
    v->u.a.size -= (j == 0 ? count : v->u.a.size - index); 
}
...
void lept_set_object_keyvalue(lept_value* v, const char* key, const lept_value* e) {
    assert(v != NULL);
}