...
#define TEST_ROUNDTRIP(json) \
    do { \
        ...
        size_t length; \
        EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, json)); \
        EXPECT_EQ_INT(LEPT_STRINGIFY_OK, lept_stringfy(&v, &json2, &length)); \
...
static void test_access_object() {
    lept_value v;
    lept_init(&v);
    lept_parse(&v, "{\"name\": \"chengkun\"}");
    EXPECT_EQ_STRING("chengkun", lept_get_string(lept_get_object_value(&v, 0)), 8);
    EXPECT_EQ_SIZE_T(8,lept_get_string_length(lept_get_object_value(&v, 0)));
    lept_free(&v);
}
